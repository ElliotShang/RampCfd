...
func muscl(kappa float64, order float64, i int, j int, mesh *common.SpaceMesh, limiterLP common.PrimtiveFlux, limiterLN common.PrimtiveFlux,
	...
	if sign == 'x' {
		...
		sumL := minusfluxL.FluxPlus(plusfluxL).ScalarMultiFlux(0.25)
		...
		sumR := minusfluxR.FluxPlus(plusfluxR).ScalarMultiFlux(0.25)
...
func LimiterCalc(mesh *common.SpaceMesh, i int, j int, sign rune) common.PrimtiveFlux {
	...
	if sign == '+' {
		...
		denumerator := primiVarR.FluxDivision(primiVarL1)
		limiterFlux = numerator.FluxDivision(denumerator)
		limiterFlux = common.LimiterFun(limiterFlux)
	}
	return limiterFlux
